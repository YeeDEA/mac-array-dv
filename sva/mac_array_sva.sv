...
module mac_array_sva #(parameter int N = 4) (
  input logic clk, rst_n,
  input logic in_valid, in_ready, in_last,
  input logic [8*N-1:0] a_col, b_row,
  input logic out_valid, out_ready, out_last,
  input logic [$clog2(N)-1:0] out_row,
  input logic [32*N-1:0] c_row,
  input logic en, clr,
  input logic signed [31:0] acc [N][N],
  input logic       state,        // ctrl FSM: 0 = S_ACCEPT, 1 = S_DRAIN
  input logic [$clog2(N)-1:0] row // ctrl drain row counter
);
  localparam int RW = $clog2(N);
  localparam int NN = N * N;
  logic [32*NN-1:0] acc_flat;
  always_comb
    for (int i = 0; i < N; i++)
      for (int j = 0; j < N; j++)
        acc_flat[(N*i+j)*32 +: 32] = acc[i][j];

  // previous-cycle samples (replacement for unsupported $past/$stable)
  logic [32*N-1:0]  c_row_q;
  logic [RW-1:0]    out_row_q;
  logic [32*NN-1:0] acc_flat_q;
  logic [16*N:0]    in_bus_q;
  logic [8*N-1:0]   a_col_q, b_row_q;
  logic         en_q, clr_q;
  wire  [16*N:0]    in_bus = {a_col, b_row, in_last};
  ...
  logic [32*NN-1:0] acc_exp;
  always_comb
    for (int i = 0; i < N; i++)
      for (int j = 0; j < N; j++)
        acc_exp[(N*i+j)*32 +: 32] =
            $signed(acc_flat_q[(N*i+j)*32 +: 32])
  ...
  // carries in_last is accepted, exactly N drain rows are owed before the tile is closed.
  ...
  wire tile_end = out_hs && out_last;      // drain of row N-1 handshaked -> clear
  logic [RW:0] rows_owed;           // 0..N
  always_ff @(posedge clk)
    if (!rst_n)                 rows_owed <= '0;
    else if (in_hs && in_last)  rows_owed <= (RW+1)'(N);
    else if (out_hs && rows_owed != 0) rows_owed <= rows_owed - 1'b1;

  // A1 (rewritten, E6): while any of the N result rows is still owed, no new input beat
  // may be accepted, the output side must be offering a row, and out_last must mark exactly
  // the N-th row. The old A1 (out_valid |-> !in_ready) restated the 1-bit FSM decode and
  ...
  a_drain_blocks_in: assert property (@(posedge clk) disable iff (!rst_n)
    (rows_owed != 0) |-> !in_hs && out_valid && (out_last == (rows_owed == (RW+1)'(1))));
  ...
  a_row_inc: assert property (@(posedge clk) disable iff (!rst_n)
    out_valid && out_ready && !out_last |=> out_row == RW'(out_row_q + 1));
  // A4: out_last exactly on the last row (row N-1; name kept from the 4x4 build)
  a_out_last_iff_row3: assert property (@(posedge clk) disable iff (!rst_n)
    out_valid |-> (out_last == (out_row == RW'(N - 1))));
  ...
  a_reset_clear: assert property (@(posedge clk)
    !rst_n |=> acc_flat == '0 && state == 1'b0 && row == '0);
